		...
		<table class="striped">
			...
			<tbody>
				<c:forEach var="item" items="${subject}">
					<c:url value="/article/${item.name}" var="retrieveUrl" />
					<tr>
						<td><a href="${retrieveUrl}">${item.article.articleName}</a></td>
						<td>${item.article.lastModified}</td>
						<td>${item.article.editor}</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
